...
import { TimeSlot, Activity, Result, success, failure } from '../types';
...
/**
 * Validated TimeSlotRepository with automatic validation
 * Extends ValidatedBaseRepository to provide TimeSlot-specific validation
 */
export class ValidatedTimeSlotRepository extends ValidatedBaseRepository<TimeSlot> {
  protected entityType: 'timeSlot' = 'timeSlot';
  ...
  protected applyCustomFilters(timeSlots: TimeSlot[], filters: any): TimeSlot[] {
  ...
  // TimeSlot-specific validated methods
  ...
  async createTimeSlot(data: {
    ...
    try {
      ...
      const appError = createError.unknown('Failed to create time slot', error);
  ...
  async logActivity(id: string, activity: Activity | null): Promise<Result<TimeSlot>> {
    try {
      ...
      const appError = createError.unknown('Failed to log activity', error);
      errorManager.handleError(appError, { timeSlotId: id, activityId: activity?.id });
      return failure(appError);
    }
  }
  ...
  async generateDailySlots(date: string, intervalMinutes: number = 30): Promise<Result<TimeSlot[]>> {
    try {
      ...
      const appError = createError.unknown('Failed to generate daily slots', error);
      errorManager.handleError(appError, { date, intervalMinutes });
      return failure(appError);
    }
  }
  ...
  async getDailyStats(date: string): Promise<Result<{
    ...
    try {
      ...
      errorManager.handleError(appError, { date });
      return failure(appError);
    }
  }
  ...
  async getTimeSlotByTime(date: string, time: Date): Promise<Result<TimeSlot | null>> {
    try {
      ...
      const appError = createError.unknown('Failed to get time slot by time', error);
      errorManager.handleError(appError, { date, time: time.toISOString() });
      return failure(appError);
    }
  }
  ...
  async getWeeklyStats(startDate: string): Promise<Result<{
    ...
    try {
      ...
      errorManager.handleError(appError, { startDate });
      return failure(appError);
    }
  }
